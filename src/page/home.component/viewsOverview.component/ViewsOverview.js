...
function ViewsOverview(props) {
  const [views, setViews] = useState([
    6, 12, 15, 11, 23, 21, 26, 20, 24, 47, 17, 15, 12, 34, 48, 14, 26, 36, 34,
    35, 41, 21, 29, 19,
  ]);
  const [isTooltipChartOpen, setIsTooltipChartOpen] = useState(false);
  const [tooltipChartLocation, setTooltipChartLocation] = useState([20, 60]);
  const [timeViews, setTimeViews] = useState(null);
  const chartLine = useRef(null);

  function showTooltipInfo(e, start, end, amount) {
    setIsTooltipChartOpen(true);
    setTooltipChartLocation([e.screenX - 300, e.screenY - 400]);
    setTimeViews(amount);
  }

  function renderToolTipChart(views) {
    let left = tooltipChartLocation[0];
    let top = tooltipChartLocation[1];
  ...
  function removeToolTipChart() {
    setIsTooltipChartOpen(false);
  }
      ...
      <div className="box box-chart-lines">
        ...
        <div className="chart-lines">
          <svg ref={chartLine}>
            ...
            <g fill="#0099cc">
              {views.map((current, ind, arr) => {
                if (ind % 4 === 0) {
                  let formatedAmount =
                    280 -
                    (current + arr[ind + 1] + arr[ind + 2] + arr[ind + 3]) -
                    20;
                  let amount =
                    current + arr[ind + 1] + arr[ind + 2] + arr[ind + 3];
                  return (
                    <circle
                      ...
                      r="6"
                      onMouseMove={(e) =>
                        showTooltipInfo(e, ind, ind + 3, amount)
                      }
                      onMouseOut={removeToolTipChart}
                      key={ind}
                    />
                  );
                }
